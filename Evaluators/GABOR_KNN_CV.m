...
for i=1:size(gaborTestingImages,1)
    ...
    classificationResult1(i,1) = KNNTesting(testnumber,modelKNN, 1);
end
%% Evaluation #1
...
for i=1:size(gaborTestingImages,1)
    edIm = reshape(gaborTestingImages(i,:), 27, []);
    testnumber= gaborTestingImages(i,:);
    classificationResult2(i,1) = KNNTesting(testnumber,modelKNN, 1);
end
...
gaborTrainingImages = convertToGabor(trainingG2G3.images, 'canny');
gaborTrainingLabels = trainingG2G3.labels;

gaborTestingImages = convertToGabor(testingG1.images, 'canny');
...
for i=1:size(gaborTestingImages,1)
    edIm = reshape(gaborTestingImages(i,:), 27, []);
    testnumber= gaborTestingImages(i,:);
    classificationResult3(i,1) = KNNTesting(testnumber,modelKNN, 1);
end
%% Evaluation #3
